...
import { ComponentType, createElement, FC, forwardRef, ReactElement } from 'preact/compat';

import { ClassNameFormatter, cn, NoStrictEntityMods } from './classname';
import { classnames } from './classnames';

const __DEV__ = import.meta.env.mode !== 'PROD';
...
export function createClassNameModifier<T>(blockName: string, mod: NoStrictEntityMods) {
	return withBemMod<T>(blockName, mod, {
		__passToProps: false,
		__simple: true
	});
}
...
export type Composition<T> = <U extends ComponentType<any>>(
	fn: U
) => FC<JSX.LibraryManagedAttributes<U, ExtractProps<U>> & T & JSX.IntrinsicAttributes>;
...
function composeSimple(mods: any[]) {
	...
	for (const { __mod, __value, __passToProps } of mods) {
	...
	return (Base: ComponentType<any>) => {
		...
		if (__DEV__) {
			const allModsFormatted = Object.keys(allMods)
				.map((key) => {
					const mods =
						allMods[key].length > 3
							? ` ${allMods[key].length} mods`
							: allMods[key].join('|');
